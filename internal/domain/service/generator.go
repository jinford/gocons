...
type codeGenerater struct {
	appName             string
	needReturnValueType bool
}

func NewCodeGenerator(appName string, needReturnValueType bool) CodeGenerater {
	return &codeGenerater{
		appName:             appName,
		needReturnValueType: needReturnValueType,
	}
}
...
func (g *codeGenerater) GenerateCode(pkgName string, structs []*entity.Struct) ([]byte, error) {
	...
	for _, s := range structs {
		f.Add(jen.Id(s.GenerateConstructorStatement(g.needReturnValueType)).Line())
